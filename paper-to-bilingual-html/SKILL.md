...
3. 提取插图与表格截图。
   - 运行 `python3 scripts/extract_figures.py PAPER.pdf -o <输出目录>/assets`,自动按 caption 定位并裁剪 Figure/Table 区域,生成 PNG 与 `manifest.json`。
...
5. 按 [references/output-format.md](references/output-format.md) 写入 HTML。
...
7. 未经用户明确要求,不要额外添加摘要、评论、难点解析、学习建议、背景扩展或结尾总结。
